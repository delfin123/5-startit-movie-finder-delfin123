...
describe('cast block', async function () {
    it('should show atleast one actor', async function () {
        ...
        .count()+' '+ 'и имя первого актера ' + await $$('.col-md-3 .text-center a').first().getText())         //one photo of the actors who are shot in this movie          
...
describe('reviews block', function () {
    it('should be atleast one review', async function () {
        ...
        .count()+' и текст первого ревью такой: '+await $$('.text-justify').first().getText())                  //output a review which on the page is shown first in the console  
    })
    ...
    it('should have reviewer name as link to source', async function () {
        ...
        await winHandles.then(async function(handles) {
            ...
            logger.info('Тест пршел успешно, данная страница доступна и полное ревью фильма с внешней интернет страницы ревьюера такой: '+ await $('.sub-heading+p').getText())      //output the full review in the console            
            await browser.close()
